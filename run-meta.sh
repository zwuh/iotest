...
func_set_default_dir "nfs"
func_set_default_dir "fuse"
func_set_default_dir "local"

N_ITER="1000 100 10 1"
for n_loops in $N_ITER
do
 func_status_line "Meta: nfs x $n_loops"
 sh ./meta.sh "nfs" $n_loops   >> $OUT  2>>$ERR
 func_status_line "Meta: iscsi x $n_loops"
 sh ./meta.sh "iscsi" $n_loops >> $OUT  2>>$ERR
 func_status_line "Meta: fuse x $n_loops"
 sh ./meta.sh "fuse" $n_loops  >> $OUT  2>>$ERR
# func_status_line "Meta: local x $n_loops"
# sh ./meta.sh "local" $n_loops  >> $OUT  2>>$ERR
done
