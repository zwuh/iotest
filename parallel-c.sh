...
func_process_arg ()
{
  ...
  do
    case $f in
      ...
      q) quiet="true" ;;
      *) cat <<\EOF
Usage: parallel-c.sh [-r|-w] [-n n_threads] [-t target_dir]
...
func_print_conf ()
{
 if test -z "$quiet"; then
  ...
  echo \ target: $target
  echo \ ext_flags to pthread: $ext_flags
  echo \ read/write: $rw
 fi
}
...
then
 echo ERR parallel-c.sh does not support dry mode.
 exit
fi
...
if test "$rw" = "write"; then
 $PTHREAD -q -w -n $n_threads -b $block_size -s $n_blocks -t $target \
          $sync_sw $direct_sw 1>>$TMPF 2>&1
else
 $PTHREAD -q -r -n $n_threads -b $block_size -s $n_blocks -t $target \
          $sync_sw $direct_sw 1>>$TMPF 2>&1
fi
